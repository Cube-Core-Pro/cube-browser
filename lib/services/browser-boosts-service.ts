 ...
 * - Pages management within collections
...
const log = logger.scope('BrowserBoosts');
...
const BackendCollectionsAPI = {
  async getAllCollections(): Promise<BackendCollection[]> {
    try {
      return await invoke<BackendCollection[]>('get_all_collections');
    } catch (error) {
      log.warn('Backend get_all_collections failed:', error);
      return [];
    }
  },

  async getCollection(id: string): Promise<BackendCollection | null> {
    try {
      return await invoke<BackendCollection | null>('get_collection', { id });
    } catch (error) {
      log.warn('Backend get_collection failed:', error);
      return null;
    }
  },

  async getRootCollections(): Promise<BackendCollection[]> {
    try {
      return await invoke<BackendCollection[]>('get_root_collections');
    } catch (error) {
      log.warn('Backend get_root_collections failed:', error);
      return [];
    }
  },

  async getChildCollections(parentId: string): Promise<BackendCollection[]> {
    try {
      return await invoke<BackendCollection[]>('get_child_collections', { parentId });
    } catch (error) {
      log.warn('Backend get_child_collections failed:', error);
      return [];
    }
  },

  async createCollection(collection: Omit<BackendCollection, 'id' | 'created_at' | 'updated_at'>): Promise<BackendCollection> {
    try {
      return await invoke<BackendCollection>('create_collection', { collection });
    } catch (error) {
      log.warn('Backend create_collection failed:', error);
      throw error;
    }
  },

  async updateCollection(collection: BackendCollection): Promise<void> {
    try {
      await invoke<void>('update_collection', { collection });
    } catch (error) {
      log.warn('Backend update_collection failed:', error);
      throw error;
    }
  },
  ...
  async deleteCollection(id: string): Promise<void> {
    try {
      ...
      log.warn('Backend delete_collection failed:', error);
      throw error;
    }
  },
  ...
  async moveCollection(id: string, newParentId: string | null): Promise<void> {
    try {
      await invoke<void>('move_collection', { id, newParentId });
    } catch (error) {
      log.warn('Backend move_collection failed:', error);
    }
  },

  async getFavoriteCollections(): Promise<BackendCollection[]> {
    try {
      return await invoke<BackendCollection[]>('get_favorite_collections');
    } catch (error) {
      log.warn('Backend get_favorite_collections failed:', error);
      return [];
    }
  },

  async getCollectionPages(collectionId: string): Promise<BackendPage[]> {
    try {
      return await invoke<BackendPage[]>('get_collection_pages', { collectionId });
    } catch (error) {
      log.warn('Backend get_collection_pages failed:', error);
      return [];
    }
  },

  async addPage(page: Omit<BackendPage, 'id' | 'created_at' | 'updated_at'>): Promise<BackendPage> {
    try {
      return await invoke<BackendPage>('add_page', { page });
    } catch (error) {
      log.warn('Backend add_page failed:', error);
      throw error;
    }
  },

  async updatePage(page: BackendPage): Promise<void> {
    try {
      await invoke<void>('update_page', { page });
    } catch (error) {
      log.warn('Backend update_page failed:', error);
      throw error;
    }
  },
  ...
  async deletePage(id: string): Promise<void> {
    try {
      ...
      log.warn('Backend delete_page failed:', error);
      throw error;
    }
  },

  async trackPageVisit(pageId: string): Promise<void> {
    try {
      await invoke<void>('track_page_visit', { pageId });
    } catch (error) {
      log.warn('Backend track_page_visit failed:', error);
    }
  },

  async getFavoritePages(): Promise<BackendPage[]> {
    try {
      return await invoke<BackendPage[]>('get_favorite_pages');
    } catch (error) {
      log.warn('Backend get_favorite_pages failed:', error);
      return [];
    }
  },

  async getRecentPages(limit: number = 20): Promise<BackendPage[]> {
    try {
      return await invoke<BackendPage[]>('get_recent_pages', { limit });
    } catch (error) {
      log.warn('Backend get_recent_pages failed:', error);
      return [];
    }
  },

  async createShare(collectionId: string, permissions: string): Promise<BackendShare> {
    try {
      return await invoke<BackendShare>('create_share', { collectionId, permissions });
    } catch (error) {
      log.warn('Backend create_share failed:', error);
      throw error;
    }
  },
};
...
// Export backend API
export { BackendCollectionsAPI };
export type { BackendCollection, BackendPage, BackendShare };
