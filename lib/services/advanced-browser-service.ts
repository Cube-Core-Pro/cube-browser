 ...
 * - Tab management (create, close, navigate, reload)
 * - Tab info and state
 * - Tab groups and pinning
 * - JavaScript execution
 * - Browser proxy operations
 * 
 * Features:
...
const BackendBrowserAPI = {
  async createTab(url: string): Promise<string> {
    try {
      ...
      log.warn('Backend create_browser_tab failed:', error);
      throw error;
    }
  },
  ...
  async closeTab(tabId: string): Promise<void> {
    try {
      ...
      log.warn('Backend close_browser_tab failed:', error);
      throw error;
    }
  },
  ...
  async navigateTab(tabId: string, url: string): Promise<void> {
    try {
      ...
      log.warn('Backend navigate_tab failed:', error);
      throw error;
    }
  },
  ...
  async reloadTab(tabId: string): Promise<void> {
    try {
      await invoke<void>('reload_tab', { tabId });
    } catch (error) {
      log.warn('Backend reload_tab failed:', error);
    }
  },

  async goBack(tabId: string): Promise<void> {
    try {
      await invoke<void>('tab_go_back', { tabId });
    } catch (error) {
      log.warn('Backend tab_go_back failed:', error);
    }
  },

  async goForward(tabId: string): Promise<void> {
    try {
      await invoke<void>('tab_go_forward', { tabId });
    } catch (error) {
      log.warn('Backend tab_go_forward failed:', error);
    }
  },

  async stopTab(tabId: string): Promise<void> {
    try {
      await invoke<void>('tab_stop', { tabId });
    } catch (error) {
      log.warn('Backend tab_stop failed:', error);
    }
  },
  ...
  async activateTab(tabId: string): Promise<void> {
    try {
      ...
      log.warn('Backend activate_tab failed:', error);
    }
  },

  async executeJS(tabId: string, code: string): Promise<void> {
    try {
      await invoke<void>('execute_js_in_tab', { tabId, code });
    } catch (error) {
      log.warn('Backend execute_js_in_tab failed:', error);
      throw error;
    }
  },

  async getTabInfo(tabId: string): Promise<BackendBrowserTab | null> {
    try {
      return await invoke<BackendBrowserTab>('get_tab_info', { tabId });
    } catch (error) {
      log.warn('Backend get_tab_info failed:', error);
      return null;
    }
  },

  async getAllTabs(): Promise<BackendBrowserTab[]> {
    try {
      return await invoke<BackendBrowserTab[]>('get_all_tabs');
    } catch (error) {
      log.warn('Backend get_all_tabs failed:', error);
      return [];
    }
  },
  ...
  async getActiveTab(): Promise<string | null> {
    try {
      return await invoke<string | null>('get_active_tab');
    } catch (error) {
      log.warn('Backend get_active_tab failed:', error);
      return null;
    }
  },
  ...
  async setTabPinned(tabId: string, pinned: boolean): Promise<void> {
    try {
      ...
      log.warn('Backend set_tab_pinned failed:', error);
    }
  },

  async setTabMuted(tabId: string, muted: boolean): Promise<void> {
    try {
      await invoke<void>('set_tab_muted', { tabId, muted });
    } catch (error) {
      log.warn('Backend set_tab_muted failed:', error);
    }
  },

  async createTabGroup(name: string, color: string): Promise<string> {
    try {
      return await invoke<string>('create_tab_group', { name, color });
    } catch (error) {
      log.warn('Backend create_tab_group failed:', error);
      throw error;
    }
  },
  ...
  async addTabToGroup(tabId: string, groupId: string): Promise<void> {
    try {
      await invoke<void>('add_tab_to_group', { tabId, groupId });
    } catch (error) {
      log.warn('Backend add_tab_to_group failed:', error);
    }
  },

  async removeTabFromGroup(tabId: string): Promise<void> {
    try {
      await invoke<void>('remove_tab_from_group', { tabId });
    } catch (error) {
      log.warn('Backend remove_tab_from_group failed:', error);
    }
  },
};

// Export backend API
export { BackendBrowserAPI };
export type { BackendBrowserTab };
 ...
 */
const DEFAULT_CONTAINERS: BrowserContainer[] = [
  {
    id: 'personal',
    name: 'Personal',
    icon: '👤',
 ...
 */
const DEFAULT_GESTURES: MouseGesture[] = [
...
export class ResourceLimiterService {
  ...
  private handleLimitExceeded(exceeded: string[]): void {
    switch (this.limits.onExceed) {
      case 'notify':
        log.warn('Resource limits exceeded:', exceeded.join(', '));
        break;
      case 'throttle':
        // Reduce refresh rates, pause animations
        log.debug('Throttling browser to reduce resource usage');
        break;
      case 'suspend':
        // Suspend background tabs
        log.debug('Suspending background tabs');
        break;
      case 'kill':
        // Kill highest resource tabs
        log.debug('Killing resource-heavy tabs');
        break;
    }
  }
  ...
  /**
   * Get hot tabs (resource hogs)
   */
  async getHotTabs(): Promise<HotTab[]> {
    // In production, analyze actual tab resources
  ...
  private registerDefaultActions(): void {
    ...
    for (const action of defaultActions) {
      this.actions.set(action.id, {
        ...action,
        action: () => log.debug(`Execute: ${action.title}`),
      });
    }
  }
      ...
      for (const tab of options.tabs) {
        ...
        if (score > 0) {
          results.push({
            ...
            subtitle: tab.url,
            icon: '📑',
            action: () => log.debug(`Switch to tab: ${tab.id}`),
            score: score + 50,
          });
        }
      }
      ...
      for (const bookmark of options.bookmarks) {
        ...
        if (score > 0) {
          results.push({
            ...
            icon: '⭐',
            action: () => log.debug(`Open bookmark: ${bookmark.url}`),
            score: score + 30,
          });
        }
      }
      ...
      for (const item of options.history) {
        ...
        if (score > 0) {
          results.push({
            ...
            subtitle: item.url,
            icon: '🕐',
            action: () => log.debug(`Open history: ${item.url}`),
            score: score + 20,
          });
        }
      }
